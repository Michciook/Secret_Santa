...
def login():
    try:
        ...
        if not creds or 'username' not in creds or 'password' not in creds:
            return jsonify({'error': 'Username and password is required!'}), 401
        ...
        return jsonify({'id' : user.user_id, 'name': user.username}), 200
...
def register():
    try:
        ...
        if existing_user:
            return jsonify({'error': 'User with this username or email already exists!'}), 408
